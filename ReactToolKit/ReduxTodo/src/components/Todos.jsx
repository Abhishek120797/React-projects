...
function Todos() {
  ...
  const handleSaveTodo = (id, text) => {
    const newText = text !== undefined ? text : todos.find((todo) => todo.id === id).text;
        ...
        <ul className="list-none">
          ...
          {todos.map((todo)=>{
            ...
            const handleTextChange = (event) => {
              setEditTexts({ ...editTexts, [todo.id]: event.target.value });
            }

            const handleSaveClick = () => {
              handleSaveTodo(todo.id, editTexts[todo.id]);
            }
            ...
            <input 
              type="text"
              value={todo.editMode ? 
                (editTexts[todo.id] !== undefined ? editTexts[todo.id] : todo.text) 
                : todo.text}
            ...
            <button
              onClick={todo.editMode ? handleSaveClick : () => handleToggleEditMode(todo.id)}
